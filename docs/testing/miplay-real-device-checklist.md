...
1. Put the OpenXiaoCast host, K60 and M01 on the same non-isolated LAN/VLAN.
...
5. Run `openxiaocast-miplay scan --timeout 5` from a second LAN host. The
   OpenXiaoCast identity must appear with audio support.

## 2. Cast from K60

1. Cold-start OpenXiaoCast, unlock the K60 and open its system audio-output or
   妙播 device picker.
2. Select the distinct `OpenXiaoCast` entry, start a local audio track and keep
...
Record: K60 model and HyperOS build, OpenXiaoCast commit/image digest, M01
