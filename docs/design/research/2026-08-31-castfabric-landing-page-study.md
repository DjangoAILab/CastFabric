...
## 2026-08-31 文案与能力边界复审

复审依据包括当前 `PlaybackTarget` / `DLNAOutputAdapter` / `XiaomiOutputAdapter` 实现、
ADR-0003 和 GHCR 公开镜像清单，并通过本机 CLI Proxy 分别请 Kimi K3、DeepSeek V4 Pro
做中文文案审校。模型仅作为语言顾问，所有建议仍逐条对照代码和数据契约。

两轮建议一致指出：

- “为……而生”“编织”“我们选择说清楚”属于低信息量营销或元话语，应直接陈述事实；
- “无需账号”必须限定为核心标准链路，因为 Xiaomi MiNA 兼容输出需要小米账号；
- “多协议、同一出口”会被误读为全局单一音响，应拆开接收协议、每音响独立入口和输出
  适配器；
- 延迟标题应该直接给出两条链路的差异，不先表扬项目愿意披露限制。

模型建议中“展示播放内容”超出控制台数据契约，已明确拒绝。最终首页只展示可验证的
发送设备、接收协议、输出适配器和设备范围；不识别来源 App，不推测曲目。

首屏能力边界锁定为：

1. **已实现接收：** DLNA、AirPlay、MiPlay；
2. **已实现核心输出：** 标准 UPnP/DLNA MediaRenderer；
3. **已实现可选输出：** Xiaomi MiNA，默认关闭，面向没有原生 DLNA 的旧小米音箱；
4. **未来边界：** 只说明 `PlaybackTarget` 可增加适配器，不列出或暗示未实现协议。

部署入口同样遵守“复制即可执行”：Hero 只链接公开 GHCR，部署区提供完整 `docker run`
命令；不再单独显示缺少 `compose.yaml` 前提的 `docker compose up -d`。
