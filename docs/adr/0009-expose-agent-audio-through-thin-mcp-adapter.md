...
1. 在现有应用服务与运行时对象之上增加薄 MCP 适配层，不直接调用 DLNA、MiNA、
   AirPlay 或 MiPlay 的协议实现。
...
5. 文件通过 MCP 以 Base64 传入，保存为仅服务当前播放的临时 HTTP 文件，再复用
   `PlaybackTarget.play_url()`；不建立持久资产库。
6. 实时入口固定接收 PCM `s16le / 48 kHz / 双声道`。MCP 工具只创建会话并返回
   WebSocket 写入地址；二进制 PCM 帧经现有 `CastFabricLiveAudioSink.write()` 进入输出。
7. 配套发布 `castfabric` Agent Skill。Skill 中的 Node.js helper 负责读取本地文件、
   Base64 封装、调用 MCP、使用 FFmpeg 把文件/URL/stdin 转为标准 PCM，以及在客户端
   进程中进行顺序或循环播放。
...
- Base64 文件调用有额外传输开销，不适合超大媒体；大文件优先使用可访问 URL。
